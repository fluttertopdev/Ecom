<?php
$categories = \Helpers::getAllCategoryList();
$langList = \Helpers::getAllLangList();
$data = \Helpers::getHeaderData();
$headlines = setting('headline');
?>
<style>
    .menu-topbar {
        display: flex;
...
<div class="topbar top-brand-2">
    <div class="container-topbar">
        ...
        <div class="info-topbar text-center d-none d-xl-block">
            <marquee behavior="alternate" onmouseout="this.scrollAmount=2" onmouseover="this.scrollAmount=0" scrollamount=2>
                @foreach($headlines as $headline)
                <a class="headline-item">
                    <span class="font-xs color-brand-3">{{ $headline }}</span>
                </a>
                @endforeach
            </marquee>
        </div>
        <div class="menu-topbar-right"><span class="font-xs color-brand-3">{{__('lang.Need_help_?_Cal_ Us')}}:</span><span class="font-sm-bold color-success"> {{ setting('helpline') }}</span>
            ...
            <div class="dropdown dropdown-language">
                <button class="btn dropdown-toggle" id="dropdownPage" type="button" data-bs-toggle="dropdown" aria-expanded="true" data-bs-display="static">
                    <span class="dropdown-right font-xs color-brand-3">{{ strtoupper($data['languageCode']) }}</span>
                </button>
        ...
        <div class="main-header">
            <div class="header-left">
                <div class="header-logo">
                    <a href="{{url('/')}}"><img alt="Ecom" src="{{url('uploads/setting/'.setting('logo'))}}"></a>
                </div>
                ...
                <div class="header-shop">
                    ...
                    <a class="font-lg icon-list icon-wishlist" href="{{ url('my-account') }}">
                        ...
                        <span class="number-item font-xs wishlistCount">{{ $data['wishlistcount'] }}</span>
                        @else
                        <span class="number-item font-xs wishlistCount" style="display: none;">{{ $data['wishlistcount'] }}</span>
                        @endif
                    </a>
                    @else
                    ...
                    <a href="{{ url('cart-details') }}">
                        <div class="d-inline-block box-dropdown-cart">
                            <span class="font-lg icon-list icon-cart">
                                <span>Cart</span>
                                @if($data['cartcount'] > 0)
                                <span class="number-item font-xs cartcount">{{ $data['cartcount'] }}</span>
                                @endif
                            </span>
                        </div>
                    </a>
