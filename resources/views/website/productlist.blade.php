...
<li><a class="font-xs color-gray-500" href="{{ url('/') }}">{{ ucwords(str_replace('-', ' ', $segment1)) }}</a></li>
...
<form id="" method="GET" action="{{ url('product-list/'.$segment1) }}"> 
...
<option value="low-high" <?= (isset($_GET['sort']) && $_GET['sort'] === 'low-high') ? 'selected' : '' ?>>Low to High</option>
<option value="high-low" <?= (isset($_GET['sort']) && $_GET['sort'] === 'high-low') ? 'selected' : '' ?>>High to Low</option>
</select>
...
<input type="hidden" name="brand_id" id="brand_id" value="{{ request()->get('brand_id') }}">
<!-- Smaller buttons for applying and resetting filters -->
<div class="d-inline-block">
<button style="background-color: #FD9636;" class="btn btn-primary btn-sm font-sm  font-medium" id="applyFilter" type="submit">{{__('lang.Apply')}}</button>
</div>
<div class="d-inline-block ms-2">
</form>
<a href="{{ url('product-list/'.$segment1) }}"> <button class="btn btn-outline-secondary btn-sm font-sm color-gray-900 font-medium" id="resetFilter" type="submit">{{__('lang.Reset')}}</button> </a>
</div>
...
$discontPrice = $activeVariant ? $activeVariant->price  : $each->discount;
...
$taxRates = $each->taxRates; // Assuming a relationship exists in the Product model
$totalTax = 0;
...
<div class="image-box">
<a href="{{ url('product-details/'.$each->slug) }}">
<img src="{{ $productImage }}" alt="Ecom">
</a>
</div>

<div class="info-right">
<a class="font-xs color-gray-500" href="{{ url('product-details/'.$each->slug) }}">{{ $each->brand_name }}</a><br>
<a class="color-brand-3 font-sm-bold" href="{{ url('product-details/'.$each->slug) }}">
{{ $each->name }}
...
<!--<input class="" type="text"value="{{$discontPrice }}"name="" id="mrp-price-{{ $each->id }}">-->
...
<h6 class="color-gray-900">{{__('lang.Product_Categories')}}</h6>
</div>
<div class="sidebar-content">
<ul class="list-nav-arrow">
@foreach ($categories as $category)
<li><a href="{{ url('product-list/'.$category->slug) }}">{{$category->name}}</a></li>
...
<input class="form-control max-value" type="hidden" name="max-value" id="max-price" value="{{ isset($_GET['max-value']) ? $_GET['max-value'] : 50000 }}">
</div>
</div>
</div>
...
<form action="{{ url('product-list/'.$segment1) }}" method="get" id="filter-form">
<!-- Brand dropdown -->
...
<input type="hidden" name="max-value" id="max-price-hidden" value="{{ isset($_GET['max-value']) ? $_GET['max-value'] : 50000 }}">
...
<!-- Reset Button -->
<a href="{{ url('product-list/'.$segment1) }}">
...
var maxPrice = {{ isset($_GET['max-value']) ? $_GET['max-value'] : 50000 }}; // Dynamically setting max price
...
range: true,
min: 0,
max: 50000,  // Set max price dynamically
