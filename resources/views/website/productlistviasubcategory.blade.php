...
<li>
    <a class="font-xs color-gray-500" href="{{ url('product-list/' . $categoryname) }}">
        {{ ucwords(str_replace('-', ' ', $categoryname)) }}
    </a>
</li>

<li><a class="font-xs color-gray-500">{{ ucwords(str_replace('-', ' ', $segment1)) }}</a></li>
...
<div class="container mb-40 mt-40" style="min-height:500px">
    <div class="row">
                        ...
                        <div class="d-inline-flex align-items-center">
                            <div class="d-inline-block me-2">
                                ...
                                <div class="dropdown-sort border-1-right">
                                    <form method="GET" action="{{ url('product-list-subcategory/'.$segment1) }}">
                                        <select class="form-select font-sm color-gray-900 font-medium" id="sortOptions" name="sort">
                                            ...
                                            <option value="low-high" {{ request()->get('sort') === 'low-high' ? 'selected' : '' }}>Price: Low to High</option>
                                            <option value="high-low" {{ request()->get('sort') === 'high-low' ? 'selected' : '' }}>Price: High to Low</option>
                                        </select>
                                </div>
                            </div>
                            ...
                            <div class="d-inline-block ms-2">
                                ...
                                <a href="{{ url('product-list-subcategory/'.$segment1) }}">
                                    <button class="btn btn-outline-secondary btn-sm font-sm color-gray-900 font-medium" id="resetFilter">Reset</button>
                                </a>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="card-grid-style-3">
                                <div class="card-grid-inner">
                                    @php
                                        ...
                                        $isInCart = collect($cartdata)->where('product_id', $each->id)->where('variants_id', $activeVariant->id ?? null)->isNotEmpty();
                                    ...
                                    <div class="tools">
                                        <a class="btn btn-wishlist mb-10 addwishlist {{ $isInWishlist ? 'activeheart' : '' }}"  
                                           data-productvariant_id="{{ $activeVariant->id ?? null }}" 
                                    ...
                                    <div class="info-right">
                                        ...
                                        <a class="color-brand-3 font-sm-bold" href="{{ url('product-details/'.$each->slug) }}">{{ $each->name }}</a>

                                        <div class="rating">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <i class="fa fa-star" style="color: {{ $i <= $each->averageRating ? '#FD9636' : '#d3d3d3' }};"></i>
                                            @endfor
                                            <span class="font-xs color-gray-500">({{ $each->reviewCount ?? 0 }})</span>
                                        </div>
                                        ...
                                        <span>{{ setting('including_tax') == 0 ? 'Tax Inclusive' : 'Tax Exclusive' }}</span>

                                        <div class="mt-20 box-btn-cart">
                                            <a class="btn btn-cart addtocart {{ $isInCart ? 'disabled' : '' }}" 
                                               data-variantproductid="{{ $activeVariant->id ?? null }}" 
        ...
        <div class="col-lg-3 order-first order-lg-first">
            <div class="sidebar-border mb-0 mt-2">
                <div class="sidebar-head">
                    <h6 class="color-gray-900">Product Subcategories</h6>
                </div>
                <div class="sidebar-content">
                    <ul class="list-nav-arrow">
                        @foreach ($subcategories as $subcategory)
                            <li><a href="{{ url('product-list-subcategory/'.$subcategory->slug) }}">{{ $subcategory->name }}</a></li>
                        @endforeach
                    </ul>
                </div>
            </div>
            ...
            <form action="{{ url('product-list-subcategory/'.$segment1) }}" method="get" id="filter-form">
            <!-- Brand dropdown -->
