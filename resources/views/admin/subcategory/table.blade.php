...
<div class="card-header">
    <div class="row">
        <div class="col-md-6">
            <h5>{{__('lang.admin_subcategory_list')}}</h5>
        </div>
        <div class="col-md-6">
            ...
            <tbody>
                ...
                <tr>
                 ...
                 <img src="{{ url('uploads/category/'.$row->image)}}" class="table_img"
                    ...
                    <td>
                        @if($row->is_popular == 1)
                        <a href="{{ url('admin/update-popular-column/'.$row->id) }}">
                    ...
                    <td>
                        @if($row->status == 1)
                        <a href="{{ url('admin/update-subcategory-column/'.$row->id) }}">
                            <span class="badge bg-success">{{__('lang.admin_active')}}</span></a>
                        @else
                        <a href="{{ url('admin/update-subcategory-column/'.$row->id) }}">
                            <span class="badge bg-warning">{{__('lang.admin_deactive')}}</span></a>
                    ...
                    <td>
                        <div class="inline_action_btn">
                            ...
                            <a class="delete_icon" href="javascript:void(0);"
                                onclick="showDeleteConfirmations('subcategory' , {{ $row->id }})"
                                ><i class="ti ti-trash me-1"></i>
                            </a>
                            
                                <a class="edit_icon" href="{{ url('admin/subcategory/translation/'.$row->id) }}"
                          title="Translation">
                    ...
                    <div class="offcanvas offcanvas-end" id="edit-new-record_{{$row->id}}">
                        <div class="offcanvas-header border-bottom">
                            <h5 class="offcanvas-title"
                                id="exampleModalLabel">{{__('lang.admin_edit_subcategory')}}</h5>
                            <button type="button" class="btn-close text-reset"
                                data-bs-dismiss="offcanvas" aria-label="Close"></button>
                        </div>
                        <div class="offcanvas-body flex-grow-1">
                            <form action="{{url('admin/update-subcategory')}}" method="POST"
                                ...
                                <div class="col-sm-12 mt-3">
                                    <label class="form-label"
                                        for="name">{{__('lang.admin_subcategory_name')}} <span class="required">*</span></label>
                  ...
                  <div class="modal-content">
                    <div class="modal-body">
                      ...
                      <div class="text-center mb-6">
                        ...
                        <p>Are you sure you want to delete this <strong id="SubcategoryName"></strong> Sucategory?</p>
                      </div>
                     <div class="modal-body">
        
        
        <p><strong>SubCategory have Product:</strong> <span id="productCount"></span></p>
   
        <label for="deleteReason">Please Transfer to other subcategory then delete:</label>
...
function showDeleteConfirmations(type, id) {
    ...
    $.ajax({
        url: '{{ url("admin/fatch-subcategorydata") }}',
        type: 'POST',
        data: {
            _token: '{{ csrf_token() }}',
        ...
        success: function (response) {
            ...
            $('#SubcategoryName').text(response.SubcategoryName);
            ...
            if (response.product_count == 0 && response.subcategory_count == 0) {
                ...
                $('#deleteReason').prev('label').text('Do you want to transfer the products under another  subcategory?');
            }
    ...
    if (selectedReason) {
          Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '{{ url("admin/deletesubcategory") }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
        ...
        Swal.fire(
            ...
            'You need to select a subcategory for deletion.',
            'warning'
        );
    }
}
