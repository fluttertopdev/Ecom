        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{$row->id}}</td>
                    <td>
                        <img src="{{ url('uploads/brand/'.$row->image)}}" class="table_img"
                    ...
                    <td>
                        @if($row->status == 1)
                        <a href="{{ url('admin/update-brand-column/'.$row->id) }}">
                    ...
                    <td>
                        <div class="inline_action_btn">
                                ...
                                onclick="showDeleteConfirmation('brand' , {{ $row->id }})"
                                ><i class="ti ti-trash me-1"></i>
                            </a>
                                <a class="edit_icon" href="{{ url('admin/brand/translation/'.$row->id) }}"
                          title="Translation">
                    ...
                    <div class="offcanvas offcanvas-end" id="edit-new-record_{{$row->id}}">
                        <div class="offcanvas-header border-bottom">
                            <h5 class="offcanvas-title"
                                id="exampleModalLabel">Edit Brand</h5>
                            <button type="button" class="btn-close text-reset"
                                data-bs-dismiss="offcanvas" aria-label="Close"></button>
                        </div>
                        <div class="offcanvas-body flex-grow-1">
                            <form action="{{url('admin/update-brand')}}" method="POST"
                                ...
                                <div class="col-sm-12 mt-3">
                                        <label class="form-label image_lable"
                                            for="ecommerce-category-image">Brand Image <span class="required">*</span></label>
                                        <div class="mb-3 d-flex align-items-start align-items-sm-center gap-4">
                                        <img src="{{ url('uploads/brand/'.$row->image)}}"
