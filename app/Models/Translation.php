   ...
   public static function getDetail($id)
{
    try {
        ...
        if ($data) {
            // Fetch translations where related language has 'is_default' == 1
            $data->translation = Translation::where('keyword', $data->keyword)
                ->whereHas('language', function ($query) {
                    $query->where('is_default', 1);  // Apply condition on the related language model
                })
                ->with('language')  // Include the related language model
                ->get();
        }
