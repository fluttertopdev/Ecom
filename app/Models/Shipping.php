    ...
    public static function getLists($search){
        try {
            ...
            if(isset($search['country_id']) && $search['country_id']!=''){
                $obj = $obj->where('country_id',$search['country_id']);
            } 
    
           if(isset($search['state_id']) && $search['state_id']!=''){
                $obj = $obj->where('state_id',$search['state_id']);
            } 

             if(isset($search['city_id']) && $search['city_id']!=''){
                $obj = $obj->where('city_id',$search['city_id']);
            } 

            if(isset($search['post_code']) && $search['post_code']!=''){
                $obj = $obj->where('post_code',$search['post_code']);
            } 
          
            $data = $obj->with(['Country', 'State','City'])->orderBy('id', 'DESC')->paginate($pagination)->appends('perpage', $pagination);
